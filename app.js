  ...
  connection.query(checkSql, [userId, gameId], (err, results) => {
    ...
    connection.query(insertSql, [userId, gameId], (err2, insertResult) => {
      if (err2) {
        ...
        return res.status(500).send('Server error: ' + err2.sqlMessage);
      }
...
app.get('/cart', checkAuthenticated, (req, res) => {
  ...
  const sql = `
    ...
    WHERE UG.userId = ? AND UG.purchaseDate IS NULL
    GROUP BY UG.gameId, G.title, G.price, G.image
  `;

  connection.query(sql, [userId], (err, cartItems) => {
    if (err) {
      console.error(err);
